...
async function generatePDF(data) {
  ...
  try {
    ...
    // For Vercel/AWS Lambda, configure chromium
    ...
    let participantSignatureBase64 = ''
    if (data.participant.participant_signature) {
      try {
        console.log('Fetching participant signature from:', data.participant.participant_signature)
        const response = await fetch(data.participant.participant_signature)
        if (response.ok) {
          const arrayBuffer = await response.arrayBuffer()
          const buffer = Buffer.from(arrayBuffer)
          participantSignatureBase64 = `data:image/png;base64,${buffer.toString('base64')}`
          console.log('Participant signature converted to base64')
        }
      } catch (error) {
    ...
    let guardianSignatureBase64 = ''
    if (data.guardian?.guardian_signature) {
      try {
        console.log('Fetching guardian signature from:', data.guardian.guardian_signature)
        const response = await fetch(data.guardian.guardian_signature)
        if (response.ok) {
          const arrayBuffer = await response.arrayBuffer()
          const buffer = Buffer.from(arrayBuffer)
          guardianSignatureBase64 = `data:image/png;base64,${buffer.toString('base64')}`
          console.log('Guardian signature converted to base64')
        }
      } catch (error) {
        console.error('Error fetching guardian signature:', error)
      }
    }
  
    const htmlContent = generateHTMLTemplate(data, logoBase64, qrAcknowledgementBase64, qrTermsBase64, participantSignatureBase64, guardianSignatureBase64)
...
function generateHTMLTemplate(data, logoBase64 = '', qrAcknowledgementBase64 = '', qrTermsBase64 = '', participantSignatureBase64 = '', guardianSignatureBase64 = '') {
  ...
  // Define standard conditions to filter out from "other" category
    ...
    <body>
      ...
      <div class="signature-section">
        <div class="signature-box">
          ...
          ${participantSignatureBase64 ? `<img src="${participantSignatureBase64}" style="max-height: 60px;" />` : ''}
        </div>
      </div>
      ...
      <div class="guardian-section">
        ...
        <div class="signature-box">
          ...
          ${guardianSignatureBase64 ? `<img src="${guardianSignatureBase64}" style="max-height: 50px;" />` : ''}
        </div>
      </div>
      ` : ''}
    </body>
